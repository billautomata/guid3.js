function create_horizontal_slider(){

  var width = 200
  var height = 120

  var scale = d3.scale.linear().domain([0,width]).range([3.3,10.21112])
...
function create_vertical_slider(){

  var width = 20
  var height = 320

  var scale = d3.scale.log().domain([Math.exp(0),height]).range([64,1000]).base(Math.E)
  ...
  var drag_function = function(d){
    ...
    rect_horizontal_indicator.attr('y', y)
    rect_horizontal_indicator.attr('height', height-y)
    console.log(height-y,scale(height-y))
